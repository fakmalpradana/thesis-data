...
def fig_4_7():
    ...
    ax.legend(handles=handles, frameon=True, framealpha=0.9, edgecolor="0.8", fontsize=6.5,
              loc="lower left", bbox_to_anchor=(0.01, 0.10), title="Stations / lines", title_fontsize=7)
    ...
    NOTES.append(("4.7", "UTM metre grid (northing labels rotated), legend lower-left (inset removed 13 Sep), line+point legend, north arrow, 5 km scale bar, CRS/datum note (13 Sep). Station 150 Sunter Hulu lies upstream on Kali Sunter (Jakarta Timur), outside the Jakut boundary -- state in caption. Station 170 Ancol Flushing is NOT drawn (no published coordinates; NULL in `stations`) -- state in caption; "
